...
var providerListCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		hadeContainer := container.(*framework.BeideContainer)
		// 获取字符串凭证
...
var providerCreateCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		// 检查服务是否存在
		providers := container.(*framework.BeideContainer).NameList()
		...
		log.Println("pFolder:", pFolder)
		log.Println("subFolders:", subFolders)
		if err != nil {
			return err
		}
